...
def _build_system(valid_models: List[str]) -> str:
    ...
    return (
        ...
        f"\"{_NENHUM}\".\n"
        f"- São ACESSÓRIOS (= \"{_NENHUM}\") mesmo citando o console no título: "
        "DualSense, DualShock, Joy-Con, Pro Controller, manete, tampa/faceplate, "
        "skin, base/dock, película, case.\n"
        f"- ATENÇÃO ao padrão 'NOME DE JOGO + console': é o JOGO à venda, não o "
        f"console => \"{_NENHUM}\". Ex: 'Sonic Generations nintendo 3ds', "
        "'New Super Mario Bros Nintendo DS', 'Pokemon X 3DS', 'Heavenly Sword Ps3', "
        "'Playstation All-Stars Battle Royale PS Vita' são todos JOGOS. Já "
        "'Nintendo 3DS com Pokemon X' é o CONSOLE (vem com o jogo).\n"
        f"- Console com DEFEITO declarado ('não liga', 'leitor com defeito', 'para "
        f"retirada de peças', 'drift') = \"{_NENHUM}\" — não serve de comparação "
        "de preço.\n"
